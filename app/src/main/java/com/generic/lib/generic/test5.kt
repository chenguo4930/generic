...
/**
 * 子类型 ： 任何时候如果需要的是类型 A 的值，也能够使用类型 B 的值，当作 A 的值，类型 B 就称为类型 A 的子类型
 ...
 * 如果是属性，就不一样，属性var 带有get和set方法，val带有get方法，是可以型变的
 * 因为构造方法不是那种在实例创建完成之后还能调用的方法，没有潜在的危险
 * 私有方法 private 的参数即不在 in 位置和 out 位置
 */
class Herd2<out T : Animal>(val animals: T, name: T)